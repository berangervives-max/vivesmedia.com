...
function Cell({ value }: { value: string | boolean }) {
  if (value === true) return <Check className="w-4 h-4 mx-auto" style={{ color: '#FF6B00' }} />
    ...
    <div className="min-h-screen bg-background pt-28 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-12 md:mb-16">
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#FF6B00' }}>Tarifs</p>
        ...
        <p className="text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: '#FF6B00' }}>Comparatif création de sites</p>
            ...
            <tbody>
              {comparisonRows.map((row, ri) => (
                <tr key={row.label} className={ri % 2 === 1 ? 'bg-muted/30' : 'bg-white'}>
                  ...
                  {row.values.map((v, ci) => (
                    <td key={ci} className={`p-5 text-center ${ri === 0 ? 'font-bold text-base' : 'text-muted-foreground'}`} style={ri === 0 ? { color: '#FF6B00' } : undefined}>
                      <Cell value={v} />
                    </td>
                  ))}
                </tr>
        ...
        <div className="md:hidden grid grid-cols-1 gap-4 mb-4">
          {ordered.map(f => (
            <div key={f.slug} className="rounded-2xl border border-border bg-white p-6">
              ...
              <p className="text-2xl font-bold mt-1" style={{ color: '#FF6B00' }}>{f.price}</p>
              <p className="text-xs text-muted-foreground">{f.priceNote}</p>
              {f.sub && <p className="text-xs font-medium mt-1" style={{ color: '#FF6B00' }}>{f.sub}</p>}
              <ul className="space-y-2 mt-4 mb-5">
                {f.features.map(feat => (
                  <li key={feat} className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Check className="w-3.5 h-3.5 shrink-0" style={{ color: '#FF6B00' }} /> {feat}
                  </li>
                ))}
              </ul>
        ...
        <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: '#FF6B00' }}>Deux façons de payer</p>
        ...
        <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: '#FF6B00' }}>Abonnements optionnels</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          {recurrentServices.map(s => (
            <div key={s.slug} className="rounded-2xl border border-border bg-white p-6 flex flex-col">
              ...
              <p className="text-xl font-bold mb-3" style={{ color: '#FF6B00' }}>{s.price}</p>
              <p className="text-sm text-muted-foreground leading-relaxed flex-1">{s.desc}</p>
              <Link href={`/services/${s.slug}`} className="mt-4 text-xs font-medium inline-flex items-center gap-1 hover:gap-2 transition-all" style={{ color: '#FF6B00' }}>
          ...
          {tarifsFaq.map(f => (
            <details key={f.q} className="group py-5">
              <summary className="flex items-center justify-between cursor-pointer list-none">
                ...
                <span className="shrink-0 text-muted-foreground transition-transform group-open:rotate-45" style={{ color: '#FF6B00' }}>
