...
export default function StatsPage() {
  ...
  const KPIS = [
    {
      label: 'Argent encaissé (12 mois)',
      value: `${caTotal12m.toLocaleString('fr-FR')} €`,
      icon: Euro,
      explication: 'Tout ce qui a été réellement payé : factures réglées + commandes Stripe. Pas les devis, pas les promesses — le cash.',
    },
    {
      label: 'Demandes de devis ce mois-ci',
      value: `${devisCeMois}`,
      icon: Target,
      explication: devisMoisPrec > 0
        ? `Le mois dernier : ${devisMoisPrec}. ${devisCeMois >= devisMoisPrec ? 'Tu progresses ↗' : 'En baisse ↘ — relance ta visibilité.'}`
        : 'Chaque demande = un prospect qui a rempli le formulaire du site.',
    },
    {
      label: 'Devis transformés en clients',
      value: tauxConversion !== null ? `${tauxConversion}%` : '—',
      icon: Percent,
      explication: tauxConversion !== null
        ? 'Sur les devis acceptés ou refusés, ce % a dit oui. Au-dessus de 30% = très bon pour le web.'
        : 'S\'affichera dès qu\'un devis sera marqué "accepté" ou "refusé".',
    },
    {
      label: 'Contacts newsletter',
      value: newsletter.length,
      icon: Mail,
      explication: 'Des prospects qui ont donné leur email volontairement — ta liste pour les campagnes. C\'est un actif qui t\'appartient.',
    },
  ]
