...
const DEFAULT_LINKS: AgendaLinks = {
  ...
  calcomUrl: 'https://cal.com/vivesmediacom',
...
export default function AgendaPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Kpis items={[
        { label: 'RDV à venir', value: evs.length, icon: CalendarDays, color: 'var(--cms-brand)' },
        { label: 'Cette semaine', value: cetteSemaine, icon: Clock, color: 'var(--cms-info-fg)' },
        { label: "Aujourd'hui", value: aujourdhui, icon: CheckCircle2, color: 'var(--cms-ok-fg)' },
      ]} />

      {/* Prochains RDV (Google Calendar via compte de service) */}
      <div className="rounded-xl p-5" style={{ background: 'var(--cms-card)', border: '1px solid var(--cms-border)' }}>
        <div className="flex items-center gap-2 mb-4">
          <CalendarDays className="w-4 h-4" style={{ color: ORANGE }} />
          <h2 className="font-bold text-sm" style={{ color: 'var(--cms-ink)' }}>Prochains rendez-vous</h2>
        </div>

        {rdv === null && <p className="text-sm" style={{ color: 'var(--cms-muted)' }}>Chargement…</p>}

        {rdv?.ok && rdv.events.length === 0 && (
          <p className="text-sm" style={{ color: 'var(--cms-muted)' }}>Aucun RDV à venir dans ton agenda Google.</p>
        )}
              ...
              return (
                <div key={e.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: 'var(--cms-ink)' }}>{e.title}</p>
                    <p className="text-xs" style={{ color: 'var(--cms-muted)' }}>{dateLabel}{e.location ? ` · ${e.location}` : ''}</p>
                  </div>
                  {e.hangoutLink && (
        ...
        {rdv && !rdv.ok && rdv.reason !== 'error' && (
          <div className="rounded-lg p-4 text-sm" style={{ background: 'var(--cms-warn-bg)', border: '1px solid var(--cms-warn-bg)', color: 'var(--cms-warn-fg)' }}>
            ...
            {rdv.saEmail && <p className="mt-2 font-mono text-xs px-2 py-1.5 rounded" style={{ background: 'var(--cms-card)', color: 'var(--cms-ink)', border: '1px solid var(--cms-warn-bg)' }}>{rdv.saEmail}</p>}
        ...
        {rdv && !rdv.ok && rdv.reason === 'error' && (
          <p className="text-sm" style={{ color: 'var(--cms-muted)' }}>Agenda Google indisponible pour le moment. Tes outils de réservation ci-dessous restent fonctionnels.</p>
        )}
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {TOOLS.map(tool => (
          <div key={tool.name} className="rounded-xl p-5 flex flex-col" style={{ background: 'var(--cms-card)', border: '1px solid var(--cms-border)' }}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 min-w-0">
                <tool.Logo size={26} className="shrink-0" />
                <p className="text-sm font-bold truncate" style={{ color: 'var(--cms-ink)' }}>{tool.name}</p>
              </div>
              {tool.connected ? (
                <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: 'var(--cms-ok-bg)', color: 'var(--cms-ok-fg)' }}>
                  <CheckCircle2 className="w-2.5 h-2.5" /> Connecté
                </span>
              ) : (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: 'var(--cms-warn-bg)', color: 'var(--cms-warn-fg)' }}>
                  À configurer
                </span>
              )}
            </div>
            <p className="text-xs leading-relaxed flex-1 mb-3" style={{ color: 'var(--cms-ink-2)' }}>{tool.role}</p>
            <div className="flex flex-col gap-1.5">
              <a href={tool.href} target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: tool.connected ? ORANGE : 'var(--cms-muted)' }}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* Calendly embarqué */}
        <div className="lg:col-span-2 rounded-xl overflow-hidden" style={{ background: 'var(--cms-card)', border: '1px solid var(--cms-border)' }}>
          <div className="px-6 pt-5 pb-3">
            <div className="flex items-center gap-2">
              <CalendarDays className="w-4 h-4" style={{ color: ORANGE }} />
              <h2 className="font-bold text-sm" style={{ color: 'var(--cms-ink)' }}>Ta page de réservation Calendly en direct</h2>
            </div>
            <p className="text-xs mt-1" style={{ color: 'var(--cms-muted)' }}>Exactement ce que voient tes prospects</p>
          </div>
          <iframe
        ...
        <div className="rounded-xl p-6" style={{ background: 'var(--cms-card)', border: '1px solid var(--cms-border)' }}>
          <h2 className="font-bold text-sm mb-1" style={{ color: 'var(--cms-ink)' }}>Types de RDV recommandés</h2>
          <p className="text-xs mb-4" style={{ color: 'var(--cms-muted)' }}>À créer dans tes outils si absent</p>
          <div className="space-y-3">
            {RDV_TYPES.map(t => (
              <div key={t.label} className="p-3.5 rounded-lg" style={{ background: 'var(--cms-surface-2)', border: '1px solid var(--cms-surface-2)' }}>
                <div className="flex items-center gap-2 mb-1">
                  <t.icon className="w-3.5 h-3.5" style={{ color: ORANGE }} />
                  <p className="text-sm font-semibold flex-1" style={{ color: 'var(--cms-ink)' }}>{t.label}</p>
                  <span className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ background: 'var(--cms-card)', color: 'var(--cms-ink-2)', border: '1px solid var(--cms-border)' }}>
                    <Clock className="w-2.5 h-2.5" /> {t.duree}
                  </span>
                </div>
                <p className="text-xs leading-relaxed" style={{ color: 'var(--cms-muted)' }}>{t.desc}</p>
              </div>
            ))}
          </div>

          <div className="mt-5 pt-4" style={{ borderTop: '1px solid var(--cms-surface-2)' }}>
