...
export default function CmsLoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--cms-ink)' }}>
      <div className="w-full max-w-sm">

        {/* Logo */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4"
            style={{ background: 'var(--cms-brand)' }}>
            <span className="text-white font-bold text-lg">V</span>
          </div>
          <h1 className="font-bold text-xl text-white tracking-tight">
            vives<span style={{ color: 'var(--cms-brand)' }}>media</span><span style={{ color: 'rgba(255,255,255,.3)' }}>.com</span>
          </h1>
        ...
        <div className="rounded-2xl p-6 space-y-4" style={{ background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.08)' }}>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {error && (
              <div className="px-4 py-3 rounded-lg text-sm" style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', color: 'var(--cms-danger-fg)' }}>
                {error}
              </div>
            )}
