...
export default function BeforeAfter({ before, after, alt }: { before: string; after: string; alt: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Poignée */}
      <div className="absolute inset-y-0 -ml-0.5 w-1" style={{ left: `${pos}%`, background: '#FF6B00' }}>
        <span className="absolute top-1/2 left-1/2 flex h-9 w-9 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-white shadow-lg" style={{ background: '#FF6B00' }}>
